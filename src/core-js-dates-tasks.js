...
function getNextFriday(date) {
  let addDays = 12 - new Date(date).getUTCDay();
  if (addDays > 7) {
    addDays -= 7;
  }
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = (date - first) / 1000 / 60 / 60 / 24;
  let weeks = Math.ceil((diffInDays + first.getDay()) / 7);
  if (first.getDay() === 0) {
    weeks += 1;
  }
  return weeks;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDate = new Date(period.start.split('-').reverse().join(','));
  const schedule = [];
  let workCount = 1;
  let nextDay = 1;
  while (currentDate <= end) {
    if (countWorkDays >= workCount) {
      schedule.push(currentDate.toLocaleDateString().split('.').join('-'));
      workCount += 1;
      nextDay = 1;
    } else {
      workCount = 1;
